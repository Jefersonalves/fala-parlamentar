...
def get_discursos_senador(codigo_parlamentar, data_inicio, data_fim):
    ...
    discursos_url = "https://legis.senado.leg.br/dadosabertos/senador/{}/discursos?dataInicio={}&dataFim={}".format(
        str(codigo_parlamentar), data_inicio, data_fim
    )
    r = requests.get(discursos_url)
    root = ElementTree.fromstring(r.content)

    # obtenção das urls dos discursos
    links_discursos_elements = root.findall(
        "Parlamentar/Pronunciamentos/Pronunciamento/UrlTexto"
    )
    links_discursos = [
        texto.text for texto in links_discursos_elements
    ]  # variável com as urls
